# works when game ends

import random
import tkinter as tk
from tkinter import messagebox  # Import for message pop-up
...
class CardGameGUI:
    ...
    def draw_card(self):
        if not self.card_drawn and not self.game_over:
    ...
    def play_card(self, card_index):
        if 0 <= card_index < len(self.current_player.hand) and not self.game_over:
    ...
    def attack_with_unit(self, unit_index):
        if 0 <= unit_index < len(self.current_player.battlefield) and not self.game_over:
    ...
    def check_game_over(self):
        # Check if the game is over when a player's HP <= 0, disable game, and show winner.
        if self.current_player.hp <= 0 or self.opponent.hp <= 0:
            self.game_over = True
            winner = self.opponent.name if self.current_player.hp <= 0 else self.current_player.name
            ...
            # Delay the message box to ensure other events have finished
            self.root.after(100, self.show_game_over_message, winner)  # 100ms delay

    def show_game_over_message(self, winner):
        # Show the game over message after a slight delay.
        try:
            messagebox.showinfo("Game Over", f"Game over because a player's HP <= 0. {winner} wins!")
