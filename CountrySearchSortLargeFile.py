# code works with large csv files
...
class CountrySearch:
    ...
    def search_by_field(self, search_term, field):
        """
        Search for countries by a specific field (e.g., country name, language, or continent).
        Uses a generator to read through the file row by row.
        """
        results = []
        with open(self.csv_file, mode='r') as file:
            ...
            for row in reader:
                ...
                if row[field].lower() == search_term.lower():
    ...
    def search_by_country(self, country_name):
        results = self.search_by_field(country_name, 'Country')
        self.display_results(results)

    def search_by_language(self, language):
        results = self.search_by_field(language, 'Language')
        self.display_results(results)

    def search_by_continent(self, continent):
        results = self.search_by_field(continent, 'Continent')
        self.display_results(results)
